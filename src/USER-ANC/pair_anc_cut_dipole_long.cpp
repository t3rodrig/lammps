...
#include "stdlib.h"
#include "string.h"
#include "pair_anc_cut_dipole_long.h"
#include "atom.h"
#include "comm.h"
#include "neighbor.h"
...
PairANCCutDipoleLong::PairANCCutDipoleLong(LAMMPS *lmp) : Pair(lmp)
{
  single_enable = 0;
...
PairANCCutDipoleLong::~PairANCCutDipoleLong()
{
  if (allocated) {
...
void PairANCCutDipoleLong::compute(int eflag, int vflag)
{
  int i,j,ii,jj,inum,jnum,itype,jtype;
  double qtmp,xtmp,ytmp,ztmp,delx,dely,delz,evdwl,ecoul,fforce;
  double r,rsq,rcu,rinv,r2inv,ranc3inv,ranc6inv;
  ...
  double forceanc,factor_coul,factor_anc,facm1;
...
void PairANCCutDipoleLong::allocate()
{
  allocated = 1;
  int n = atom->ntypes;
...
void PairANCCutDipoleLong::settings(int narg, char **arg)
{
  if (narg < 1 || narg > 2) 
...
void PairANCCutDipoleLong::coeff(int narg, char **arg)
{
  if (narg < 5 || narg > 6) 
...
double PairANCCutDipoleLong::init_one(int i, int j)
{
...
void PairANCCutDipoleLong::init_style()
{
...
void PairANCCutDipoleLong::write_restart(FILE *fp)
{
...
void PairANCCutDipoleLong::read_restart(FILE *fp)
{
...
void PairANCCutDipoleLong::write_restart_settings(FILE *fp)
{
...
void PairANCCutDipoleLong::read_restart_settings(FILE *fp)
{
  if (comm->me == 0) {
...
void *PairANCCutDipoleLong::extract(const char *str, int &dim)
{
