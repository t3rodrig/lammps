...
#include "stdlib.h"
#include "string.h"
#include "pair_anc_cut_coul_long.h"
#include "atom.h"
#include "comm.h"
#include "force.h"
#include "kspace.h"
...
PairANCCutCoulLong::PairANCCutCoulLong(LAMMPS *lmp) : Pair(lmp)
{
...
PairANCCutCoulLong::~PairANCCutCoulLong()
{
  if (allocated) {
...
void PairANCCutCoulLong::compute(int eflag, int vflag)
{
  int i,ii,j,jj,inum,jnum,itype,jtype,itable;
  double qtmp,xtmp,ytmp,ztmp,delx,dely,delz,evdwl,ecoul,fpair;
  double fraction,table;
  double r,rsq,rcu,r2inv,ranc3inv,ranc6inv,forcecoul,forceanc,factor_coul,factor_anc;
  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    for (jj = 0; jj < jnum; jj++) {
      ...
      if (rsq < cutsq[itype][jtype]) {
        ...
        fpair = (forcecoul + factor_anc*forceanc) * r2inv;
...
void PairANCCutCoulLong::allocate()
{
  allocated = 1;
  int n = atom->ntypes;
...
void PairANCCutCoulLong::settings(int narg, char **arg)
{
...
void PairANCCutCoulLong::coeff(int narg, char **arg)
{
  if (narg < 5 || narg > 6)
...
void PairANCCutCoulLong::init_style()
{
  if (!atom->q_flag)
    error->all(FLERR,"Pair style anc/cut/coul/long requires atom attribute q");
...
double PairANCCutCoulLong::init_one(int i, int j)
{
...
void PairANCCutCoulLong::write_restart(FILE *fp)
{
...
void PairANCCutCoulLong::read_restart(FILE *fp)
{
...
void PairANCCutCoulLong::write_restart_settings(FILE *fp)
{
...
void PairANCCutCoulLong::read_restart_settings(FILE *fp)
{
  if (comm->me == 0) {
...
void PairANCCutCoulLong::write_data(FILE *fp)
{
...
void PairANCCutCoulLong::write_data_all(FILE *fp)
{
  for (int i = 1; i <= atom->ntypes; i++)
    for (int j = i; j <= atom->ntypes; j++)
      fprintf(fp,"%d %d %g %g %g %g\n",i,j,epsilon[i][j],rmin[i][j],suavi[i][j],cut_lj[i][j]);
}

/* ---------------------------------------------------------------------- */

double PairANCCutCoulLong::single(int i, int j, int itype, int jtype,
                                 double rsq,
                                 double factor_coul, double factor_anc,
                                 double &fforce)
{
  double r,rcu,r2inv,ranc3inv,ranc6inv,grij,expm2,t,erfc,prefactor;
  double fraction,table,forcecoul,forceanc,phicoul,phianc;
  int itable;
  ...
  fforce = (forcecoul + factor_anc*forceanc) * r2inv;
...
void *PairANCCutCoulLong::extract(const char *str, int &dim)
{
  dim = 0;
